...
export const initSocketClient = (username: string) =>{
    ...
    socket.on("new-message", (data:any)=>{
        //console.log("server send msg: "+ data.content);
...
export const loadGroups = (username: string) =>{
    return function(dispatch :Dispatch){
        api.get(`${urlPrefix}/group/${username}`)
        .then(res =>{
    ...
    return function(dispatch :Dispatch) {
        ...
        for(let i = 0;i< groupPosts.length;i++) {
            if(groupPosts[i].groupId == groupId) {
                if(direct == 'desc') 
                    url += `&before=${groupPosts[i].oldestPostId}`;
        ...
        .then(result => {
            if(result.data.successed) {
                let newPosts : Array<PostData>= [], latest = '', oldest= '';
                ...
                dispatch(setPostForGroup({
                    groupManagePost: groupManagePost, isNew : isNew
                }));
            }
...
export const loadCurrentPost = (post: PostData, username: string) =>{
    ...
    return function(dispatch: Dispatch){
        const lst = store.getState().chat.postMessages.filter(item => item.postId == post.id);
        if(lst.length == 0){
            api.get(`${urlPrefix}/post/${post.id}/message?limit=20&username=${username}`)
            .then(result=>{
                if(result.data.successed) {
                    const chatMessages = result.data.data.chatMessages;
                    console.log(chatMessages);
                    dispatch(setCurrentPostAndMessage({
                        post: post, messageItems: chatMessages,
                        latest: chatMessages.length>0?chatMessages[chatMessages.length -1].id:null,
...
const recieveMessage = (chatMessage: ChatMessages, type: string) =>{
        console.log(`socket recieve message type ${type}`);

        const chatState = store.getState().chat;
        //console.log(chatMessage.postId);
        //console.log(chatState.currentSessionId);
        ...
        switch(type){
            case MESSAGE_TYPE.MESSAGE_GROUP_POST:
                if(chatMessage.postId == chatState.currentSessionId) {
                    ...
                    store.dispatch(setMessageItems({messageItems:  lst, postId: chatMessage.postId
...
export const postShareResource = (params: ShareResourceParams)=>{
    try {
        api.post(urlPrefix + '/share', params)
        .then(result=>{
            if(result.data.successed) {
...
export const checkFriendShip = async (username1 : string, username2: string)=>{
    try{
        const result = await api.get(urlPrefix+`/checkfriend?username1=${username1}&username2=${username2}`)
        if(result.data.successed) {
...
export const getRelationship = async (username1: string, username2: string)=>{
    try{
        const result = await api.get(urlPrefix+`/relationship?username1=${username1}&username2=${username2}`)
        if(result.data.successed) {
            return result.data.data.relationship;
        }
        else throw Error(result.data.message);
...
export const requestFriend = async (username1: string, username2: string)=>{
    ...
    const result = await api.post(urlPrefix+'/friendreq', {senderId: username1, recieverId: username2});
    if(result.data.successed){
...
export const getFriendRequest = (username: string)=>{
    return function(dispatch: Dispatch) {
        api.get(urlPrefix + `/friendreq/to/${username}`)
        .then(result=>{
            if(result.data.successed) {
                const friendReqs :Array<FriendRequest>= result.data.data.friendReqList;
                const friendEvents : Array<FriendEvent> = friendReqs.map(freq=>(
                    {
                        type : 'FRIEND_REQUEST',
...
export const acceptFriend = (params: AcceptFriendParam)=>{
    return function(dispatch: Dispatch){
        api.post(urlPrefix + '/acceptfriend', params)
        .then(result=>{
            if(result.data.successed) {
...
export const createGroup = (username: string, groupName: string)=>{
    return function(dispatch: Dispatch){
        api.post(urlPrefix+'/group', {name: groupName, creatorId: username})
        .then(result=>{
            if(result.data.successed) {
...
export const createPostGroup = (params: CreatePostGroupParam)=>{
        ...
        api.post(urlPrefix + '/group/thread', params)
        .then(result=>{
            if(result.data.successed) {
                //const posts = [result.data.data.post,...store.getState().chat.groupPosts];
                const newPost = result.data.data.post;
                const groupManagePost =  store.getState().chat.groupPosts.filter(item => item.groupId === newPost.groupId)[0];        
...
export const addUserToGroup = (params: AddUserToGroupParam)=>{
    api.post(urlPrefix + '/group/user', params)
    .then(result=>{
        if(result.data.successed){
...
export const getGroupMembers = async(groupId: string,username: string) =>{
    ...
    api.get(urlPrefix +`/group/member?username=${username}&groupId=${groupId}` )
    .then(result => {
        if(result.data.successed) {
            store.dispatch(setGroupUsers({
                groupId: groupId, 
...
export const loadGeneralChatMsg = async (groupId: string)=>{
    if(store.getState().chat.groupGeneralMessages.filter(item=> item.groupId === groupId).length ==0){
        api.get(urlPrefix + `/group/${groupId}/message`)
        .then(result=>{
            if(result.data.successed){
