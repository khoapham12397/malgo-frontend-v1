...
const chatSlice = createSlice({
  ...
  reducers: {
    ...
    setCurrentGroup: (state, action) => {
      ...
      const grs = state.groupPosts.filter(
      ...
      if (grs.length > 0) {
        ...
        state.currentSessionId = grs[0].currentPost
          ? grs[0].currentPost.id
          : null;
        ...
        if (
          ...
          state.messageItems = state.postMessages.filter(
            item => item.postId === postId
          )[0].messages;
        } else {
          if (state.currentGroupMode === GROUP_MODE.GENERAL_CHAT) {
            //state.messageItems = state.groupGeneralMessages.filter(item => item.groupId === action.payload.groupId)[0].messages;
            //if(state.groupGeneralMessages.filter(item => item.groupId === action.payload.groupId)[])
            const groupMsg = state.groupGeneralMessages.filter(
              item => item.groupId === action.payload.groupId
            )[0];
            if (groupMsg) state.messageItems = groupMsg.messages;
            else state.messageItems = [];
          } else state.messageItems = [];
        }
      }
      state.currentPosts = state.groupPosts.filter(
        item => item.groupId === action.payload.groupId
      )[0].posts;
    },
    setCurrentPost: (state, action) => {
      ...
      const p = state.postMessages.filter(
        item => item.postId == action.payload.post.id
      );
      if (p.length > 0) {
    ...
    addGroupMember: (state, action) => {
      //console.log(
      //  `add ${action.payload.username} isonline = ${action.payload.isOnline} to group: ${action.payload.groupId}`
      //);
      for (let i = 0; i < state.groupUsers.length; i++) {
        if (state.groupUsers[i].groupId === action.payload.groupId) {
          state.groupUsers[i].users.unshift(action.payload.groupUser);
          break;
        }
      }
    },
    setGeneralChatGroupMode: (state, action) => {
      ...
      if (msgs) {
        ...
        //console.log('here');
        state.messageItems = state.groupGeneralMessages.filter(
          item => item.groupId === action.payload.groupId
        )[0].messages;
      }
      state.currentPost = null;
    ...
    setSessionP2PMessages: (state, action) => {
      if (action.payload.messages) {
        ...
        state.messageItems = state.sessionP2PMessages.filter(
    ...
    addOneMessageP2P: (state, action) => {
      for (let i = 0; i < state.sessionP2PMessages.length; i++) {
        if (
          state.sessionP2PMessages[i].sessionId ===
          action.payload.message.sessionId
        ) {
      ...
      if (state.sessionP2PList) {
        for (let j = 0; j < state.sessionP2PList.length; j++) {
          if (
            state.sessionP2PList[j].sessionId ===
            action.payload.message.sessionId
          ) {
            ...
            if (
              state.currentSessionIdP2P === action.payload.message.sessionId
            ) {
    ...
    changePageMode: (state, action) => {
      ...
      if (
        state.pageMode === GROUP_PAGE_MODE.GROUP &&
        action.payload.mode === GROUP_PAGE_MODE.CHAT_P2P
      ) {
        // group=>p2p:
      ...
      if (
        state.pageMode === GROUP_PAGE_MODE.CHAT_P2P &&
        action.payload.mode === GROUP_PAGE_MODE.GROUP
      ) {
