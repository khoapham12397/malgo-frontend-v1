    ...
    return (
      <Container>
        <div className='list-type'>
          <span
            className={'contest-type ' + (mode == 'intro' ? 'chosen-type' : '')}
            onClick={() => handleChangeMode(CONTEST_PAGE_MODE.INTRO)}
          >
            Home
          </span>
          <span
            className={'contest-type' + (mode == 'tasks' ? ' chosen-type' : '')}
            onClick={() => handleChangeMode(CONTEST_PAGE_MODE.TASKS)}
          >
            Tasks
          </span>
          <span
            className={
              'contest-type' + (mode == 'submissions' ? ' chosen-type' : '')
            }
            onClick={() => handleChangeMode(CONTEST_PAGE_MODE.SUBMISSIONS)}
          >
            Submissions
          </span>
          <span
            className={
              'contest-type' + (mode == 'standing' ? ' chosen-type' : '')
            }
            onClick={() => handleChangeMode(CONTEST_PAGE_MODE.STANDING)}
          >
            Standing
          </span>
        </div>
        ...
        {mode === CONTEST_PAGE_MODE.INTRO ? (
          ...
          <ContestTasks
            problems={contestData.codingProblems}
          ...
          <ContestStanding
            ...
            problems={contestData.codingProblems.map((item: any) => item.id)}
          />
        ) : (
          ''
        )}
