...
export class GameManager {
  ...
  constructor(
    ...
    this.posCameraY = 100;
    this.configTiles = [
      { sx: 200, sy: 880, ow: 40, oh: 20 },
      { sx: 196, sy: 457, ow: 60, oh: 30 },
      { sx: 325, sy: 324, ow: 66, oh: 33 },
  ...
  drawBackground(ctx: CanvasRenderingContext2D) {
    ...
    let finalRow = Math.floor(
      (this.posCameraY + gameConfig.cameraCfg.height) / gameConfig.tileCfg.h
    );
    let finalCol = Math.floor(
      (this.posCameraX + gameConfig.cameraCfg.width) / gameConfig.tileCfg.w
    );
  ...
  calMainCharacter() {
    if (this.mainCharacter) this.mainCharacter.calAll();
  }
