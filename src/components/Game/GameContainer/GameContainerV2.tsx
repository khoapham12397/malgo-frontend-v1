...
export class GameContainerV2 extends React.Component<any, MyState> {
  ...
  handleKeyUp(e: any) {
    if (e.keyCode == 39 || e.keyCode == 37)
      this.gameManager.mainCharacter.stopMove();
    if (e.key == 'x' && this.gameManager.mainCharacter.flyCtl.nextWall) {
      this.gameManager.mainCharacter.fly();
    }
    if (e.key == 'c') {
  ...
  initSocket() {
    if (!this.socket.current && this.myUsername) {
      ...
      this.socket.current.on('quiz-state', (data: any) => {
        if (data.matchState) {
          const socketInfo = data.matchState.socketInfos.forEach(
            (socketItem: any) => {
        ...
        if (this.state.mode == GAME_MODE.WAITING_QUIZ) {
          data.quizStateList.forEach((item: any) => {
            ...
            if (
              item.quizKey == this.state.quizKey &&
  ...
  handleChangeMode(mode: number, quizKey: string, result: number | null) {
    ...
    if (mode == GAME_MODE.QUIZ) {
      ...
      if (this.state.quizKey) {
        const quiz = (this.gameManager.gameData as GameData).quizMap.get(
          this.state.quizKey
        );
        this.setState({
          ...this.state,
      ...
      if (mode == GAME_MODE.ACTION) {
        if (this.state.mode == GAME_MODE.QUIZ) {
          this.gameManager.mainCharacter.position.x -=
            (this.gameManager.mainCharacter.dir *
              this.gameManager.mainCharacter.currentShape.w) /
            2;
          this.socket.current.emit('quiz', {
            roomId: this.state.matchInfo.roomId,
  ...
  handleStart() {
    if (!this.state.gameLoop) {
      this.setState({
        ...
        waitingGame: false,
        pageMode: GAME_PAGE_STATE.IN_MATCH
      });
    } else {
      clearInterval(this.state.gameLoop);
        ...
        <div className='main'>
          <div className='game-screen-area'>
            ...
            <div
              style={{
                ...
                width: screenWidth,
                height: screenHeight,
                display: this.state.mode == GAME_MODE.QUIZ ? 'block' : 'none'
              }}
            >
              {this.state.quizKey ? (
                <QuizPanel
          ...
          {this.state.pageMode == GAME_PAGE_STATE.IN_MATCH ? (
            ...
            >
              ...
              />
              {this.myUsername ? (
                <Button onClick={this.handleGameRequest} className='btn-start'>
                  Start
                </Button>
              ) : (
            ...
            {this.state.matchState ? (
              <Table className='score-table'>
                <thead>
                  <tr>
                    <th key={0}></th>
                    {Object.keys(this.state.matchState.quizStates).map(
                      (quizKey, index) => (
                ...
                <tbody>
                  {this.state.matchState.socketInfos.map((socket: any) => (
                    <tr key = {socket.id}>
