...
export class Trunks {
		...
		if(this.currentState != STATE.FLYING) {
			if(this.currentState == STATE.HURTING_L1){
               ...
               // this.indFrameRun += 1;
                ...
                if(this.indFrameRun < config.cnt -1){
					if(this.cntFrameRun == config.skipFrame-1) 
						this.indFrameRun +=1;
					this.cntFrameRun = (this.cntFrameRun + 1)%config.skipFrame;
				}
    ...
    draw(ctx : CanvasRenderingContext2D){
		...
		if(this.currentState == STATE.RUNNING){
            this.indFrameRun = this.indFrameRun % config.cnt;
		...
		ctx.drawImage((this.action.isAttacking)?this.imageAttack:this.image, sx, sy, config.ow, config.oh, 
