...
export class Trunks {
  ...
  constructor(gameManager: GameManager, pos: Position) {
    ...
    this.image.src = 'trunks-tiny-rm1.png';
    ...
    if (this.currentState != STATE.FLYING) {
      if (this.currentState == STATE.HURTING_L1) {
        ...
        if (this.indFrameRun < config.cnt - 1) {
          if (this.cntFrameRun == config.skipFrame - 1) this.indFrameRun += 1;
  ...
  draw(ctx: CanvasRenderingContext2D) {
    ...
    if (this.currentState == STATE.RUNNING) {
      ...
      sx = config.sx - this.indFrameRun * config.ow * this.dir;
      sy = config.sy;
    } else {
    ...
    ctx.drawImage(
      this.action.isAttacking ? this.imageAttack : this.image,
      sx,
      sy,
      config.ow,
      config.oh,
