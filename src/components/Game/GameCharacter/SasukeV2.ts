...
export class SasukeV2 {
    ...
    constructor(gameManager : GameManager, pos : Position){
        ...
        this.step = 16; this.gravity = 8;  this.vy = 40;
        this.indFrame = 0; this.dir = 1;
        this.imageLeft = new Image(); this.imageLeft.src = "sasuke-1.png";
        this.imageRight = new Image(); this.imageRight.src = "sasuke-1r.png";
        ...
        */
        ...
        // 0 1 2 3 idle
        const idleAnim:Animation = new Animation('idle',
            4, 'sasuke/idle-left.png','sasuke/idle-right.png', 81, 81, 81*this.scale,81*this.scale);
        
            const runAnim : Animation = new Animation('run',
        6,'sasuke/run-left.png','sasuke/run-right.png',81, 81, 81*this.scale,81*this.scale);
        
        const flyAnim : Animation = new Animation('fly', 2, 
        'sasuke/fly-left.png','sasuke/fly-right.png', 81,81,81*this.scale,81*this.scale);
        
        const attack1Anim: Animation = new Animation('attack-1', 
        4, 'sasuke/attack-1-left.png','sasuke/attack-1-right.png',122, 81, 122*this.scale,81*this.scale);
        
        this.animations = [
            idleAnim, runAnim, flyAnim, attack1Anim
        ];
    ...
    calPosition(){
		...
		if(this.action.isFlying){
			this.prePosY = this.position.y;
			this.position.y = this.position.y - this.vy + this.gravity * (this.flyCtl.flyTime +0.5);
		...
		if(this.action.isFalling && !this.action.isFlying) {
			this.prePosY = this.position.y;
			this.position.y = this.position.y + this.gravity*(this.flyCtl.fallingTime+ 0.5) ;
        ...
        let u = (gameConfig.cameraCfg.width-50)*0.7 , v= (gameConfig.cameraCfg.width-50)*0.4;
        ...
        this.gameManager.posCameraX = Math.min(this.mapCfg.width - gameConfig.cameraCfg.width,this.gameManager.posCameraX)
	
		
		
		let dy = this.position.y - this.gameManager.posCameraY;

        u = gameConfig.cameraCfg.height*0.75 ; v = gameConfig.cameraCfg.height*0.25;
		if(dy > u ){
    ...
    checkFreeFall(){
        ...
        let indRow = Math.floor((this.position.y + this.currentShape.h/2 +this.gravity )/ this.tileCfg.h);       
    ...
    checkOnGround(){
        ...
        if(cnt > 0) {
            this.flyCtl = {
                fallingTime: 0, flyDown: false, flyTime: 0, climbWall: this.flyCtl.climbWall,
                nextWall: this.flyCtl.nextWall, cntClimb: 0,
            }
	...
	checkWall(){
        let val= this.position.x + this.dir * (this.currentShape.w/2+this.step);
        let indCol = Math.floor(val/this.tileCfg.w);

		for(let i =  this.position.y - this.currentShape.h/4 - this.gravity; i < this.position.y + this.currentShape.h/4 -1 ; i++){
	...
	checkCollisonWall(){
		...
		let indCol = Math.floor(val/this.tileCfg.w) , t=0;
        let startRow = Math.floor((this.position.y - this.currentShape.h/4)/ this.tileCfg.h);
        let endRow = Math.floor((this.position.y + this.currentShape.h/4)/this.tileCfg.h);
	...
	checkCollisonLeftWall(){
        ...
        let indCol = Math.floor(val/ this.tileCfg.w);
		let startRow = Math.floor((this.position.y - this.currentShape.h/4)/this.tileCfg.h),
            endRow = Math.floor((this.position.y + this.currentShape.h/4)/this.tileCfg.h);
    ...
    move(){
		...
		if(this.action.isMoving) {
            if(this.currentState == STATE.ATTACKING && this.attackCtl.attackType == 4) {
	...
	calStateDirect(){
		...
		if(this.currentState == STATE.ATTACKING){
			if(this.indFrame == this.configAttackLefts[this.attackCtl.attackType].cnt){
				this.action.isAttacking = false; this.indFrame=0;
		...
		if(this.currentState == STATE.HURTING_L1){
			if(this.indFrame == this.configLefts[this.currentState].cnt){
				this.action.isHurting = false; this.indFrame = 0;
			}
		}
    ...
    checkSlashWoodBox(){
        ...
        const startFrame = Math.max(0,this.configAttackLefts[this.attackCtl.attackType].cnt -3);
        
        if(this.indFrame != startFrame) {
            return;
        }
        const col = Math.floor((this.position.x + this.dir * (this.currentShape.w/2+this.step))/gameConfig.tileCfg.w),
              rowUp = Math.floor((this.position.y - this.currentShape.h/3)/gameConfig.tileCfg.h),
              rowDown =  Math.floor((this.position.y + this.currentShape.h/3)/gameConfig.tileCfg.h);
            ...
            if(cell.col == col && (cell.row == rowUp || cell.row == rowDown)){
                ...
                if(x){
                    if(x==2) {
                        ...
                        this.gameManager.explosionList.push(new Explosion(this.gameManager, 0, {
    ...
    checkFireCard(){
        ...
        if(card) {
            //console.log("vao day");
            if(this.gameManager.gameData.quizState.get(quizKey)?.state== 0 && !this.gameManager.gameData.solvedQuizs[quizKey]) {
    ...
    checkSlashBot(){
        ...
        const maxFrame = Math.max(0,this.configAttackLefts[this.attackCtl.attackType].cnt -1);
        if(this.currentState == STATE.ATTACKING && this.indFrame == maxFrame -1 && this.attackCtl.attackType == 4) {
            const px = this.position.x + this.dir*(this.currentShape.w/2 - this.step/2),
            py = this.position.y - this.currentShape.h/4;

            this.gameManager.longKi = new LongKi(this.gameManager,{x: px,y: py},this.dir);
            this.action.isMoving = false;
        }
    ...
    checkSingleBot(bot: MettarBot) {
        const upBot = bot.position.y - bot.currentShape.h/2 + 3;
            ...
            let collis = (upBot < this.position.y + this.currentShape.h/2) &&
            ...
            if(this.dir==1) {
                ...
                collis = collis && (limitRight >= bot.position.x - bot.currentShape.w/2) && u;
                hurtColis = limitRight - this.step -(this.currentState==STATE.ATTACKING?this.currentShape.w/4:0) >= bot.position.x -  bot.currentShape.w/4  && u;
            }
            else {
                const limitLeft = this.position.x - this.currentShape.w/2 - this.step;
                const u = bot.position.x < this.position.x + this.currentShape.w/4;
                collis = collis && (limitLeft <= bot.position.x + bot.currentShape.w/2) && u;
                hurtColis = limitLeft + this.step +(this.currentState==STATE.ATTACKING?this.currentShape.w/4:0)<= bot.position.x + bot.currentShape.w/4 && u;
            }
    ...
    setBotAppear(){
        ...
        if(this.position.x > left && this.position.x < right && this.position.y > up && this.position.y < bottom){
            this.gameManager.botList.forEach(bot=>{
                if(!bot.isAlive) {
                    //console.log("hey");
    ...
    draw(ctx : CanvasRenderingContext2D){
		...
		if(this.currentState == STATE.FLYING) {
			...
			this.indFrame = this.indFrame % config.cnt;
        ...
        this.animations[this.currentState].draw(ctx,
            this.indFrame, this.dir, px, py);
        /*
		ctx.drawImage(this.dir==1?this.imageLeft:this.imageRight, sx, sy, config.ow, config.oh, 
    ...
    drawBlood(ctx: CanvasRenderingContext2D){
        ...
        if(this.blood>0) ctx.fillRect(10,10+(120 - this.blood),10, this.blood);
    }   
} 
