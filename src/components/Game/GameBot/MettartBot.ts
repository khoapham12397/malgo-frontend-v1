...
export class MettarBot {
	...
	constructor(gameManager: GameManager,pos: Position, img : HTMLImageElement, imgRight :HTMLImageElement, name: string){
		...
		STATE.IDLE = 0; STATE.RUNNING = 1; STATE.JUMPING = 3; STATE.SHOOTING = 2;
		this.indFrame = 0;
		this.currentState = 0;
		this.dir = -1;

        this.action = {
            isRunning: false, isJumping: false, isShooting: false,
            isFalling: false,
        }
		...
		this.isAlive=true;
		this.hurtCtl = {
			cntHurt: 0, hurtFrameCnt : 0, isHurting: false, maxFrame: 0,
		}
		this.name= name;
...
//		this.indFrame = 0;
	...
	shoot(){
		...
		if(this.action.isShooting ) {
			if((this.indFrame == 1 || this.indFrame == this.config[this.currentState].cnt -1) && this.cntUpdate ==0){
	...
	checkFreeFall(){
        ...
        let indRow = Math.floor((this.position.y + this.currentShape.h/2 +this.gravity )/ gameConfig.tileCfg.h);       
	...
	checkCollisonWall(){
		...
		let indCol = Math.floor(val/gameConfig.tileCfg.w) , t=0;
        let startRow = Math.floor((this.position.y - this.currentShape.h/4)/ gameConfig.tileCfg.h);
        let endRow = Math.floor((this.position.y + this.currentShape.h/4)/gameConfig.tileCfg.h);
		let row = Math.floor((this.position.y)/gameConfig.tileCfg.h);
	...
	beHurt(){
		if(this.hurtCtl.cntHurt == this.hurtCtl.maxFrame) {
			this.isAlive = false;	
			const row = Math.floor((this.position.y - this.currentShape.h/2)/gameConfig.tileCfg.h),
			col= Math.floor((this.position.x - this.currentShape.w/2)/gameConfig.tileCfg.w);
	...
	draw(ctx :CanvasRenderingContext2D){
		...
		let cfg = this.dir==-1?this.config[this.currentState]:this.configRight[this.currentState];
		...
		ctx.drawImage(this.dir==-1?this.image:this.imageRight,sx,sy,ow,oh,
			Math.round(this.position.x - this.currentShape.w/2- this.gameManager.posCameraX), 
			Math.round(this.position.y- this.currentShape.h/2- this.gameManager.posCameraY + this.gravity + 5),
			this.currentShape.w, this.currentShape.h);
		ctx.filter= "none";
