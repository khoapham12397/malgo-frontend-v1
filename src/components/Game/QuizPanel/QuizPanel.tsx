...
type Props = {
    ...
    handleChangeMode : (mode: number, quizKey: string, result: number | null) =>void;
}
type OptionProps = {
    options: Array<string>;
...
const OptionPanel = ({options, handleChoose }: OptionProps) => {
    ...
    return (
        <div style={{display: "-ms-flexbox"}}>
            ...
            <div className="option-item" key= {0}
            ...
            <div className="option-item" key= {3}
...
export const QuizPanel = ({quizType,quizKey, quizData, handleChangeMode}: Props) => {
    ...
    return (
    <div className="panel">
        <div className="close-quiz" onClick={()=>handleChangeMode(GAME_MODE.ACTION,quizKey, null)}>X</div>
        ...
        {quizType=="option"?<OptionPanel options={quizData.options}
        handleChoose = {handleChoose}/>:(
            <div style={{margin:"10px"}}>
                <input type="text"  ref={inp}/>
                <button className="btn-submit" onClick={handleSubmit}>Ok</button>
            </div>
        )}
        </div>
