...
export class LongKi{
    ...
    checkAlive(){
        ...
        this.cntFrame += 1;
        let topLeft = {x: 0, y: 0}, downBottom = {x:0,y:0};
        if(this.dir == 1){
            topLeft = {x: this.position.x , y: this.position.y};
            downBottom = {x: this.position.x+ this.width ,y:this.position.y+ this.height};
        } 
        else {
        ...
        this.gameManager.botList.forEach(bot => {
            const botTopLeft = {x: bot.position.x - bot.currentShape.w/2, y: bot.position.y - bot.currentShape.h/2},
            botDownBottom = {x: bot.position.x + bot.currentShape.w/2, y: bot.position.y + bot.currentShape.h/2};
            const collis =  !(topLeft.x > botTopLeft.x ||  botDownBottom.x < topLeft.x) &&
    ...
    draw(ctx:CanvasRenderingContext2D) {
        ...
        ctx.drawImage(this.dir == 1?this.imageLeft:this.imageRight,
            0,0,this.ow,this.oh, Math.round(px - this.gameManager.posCameraX),
            py, this.width, this.height
            );    
    }
}