...
import { loadChatSessionMessages, loadChatSessionP2PList, MESSAGE_TYPE } from "../../state/actions/chatAction";
...
export const ChatList = ()=>{
    ...
    const handleLoadChatSession = (sessionId: string, partnerId: string, unseenCnt : number)=>{
    ...
    return (<div className='side-bar-content' >
      ...
      <div style={{borderBottom : "1px solid"}}>
        {sessionP2PList?sessionP2PList.map((item : ChatSessionP2P)=>(
          <ChatListItem item={item} key={item.sessionId} handleLoadChatSession = {handleLoadChatSession}/>
        )):''}
      </div>
