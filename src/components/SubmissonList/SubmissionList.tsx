    ...
    useEffect(()=>{
        ...
        .then(result=>{
            if(result.status === 200) {
                ...
                setShowDetailArr(result.data.data.submissions.map((item:any) => false));
                setIsPolling(true);
            }
        })
        .catch(error=>{
            ...
            <div >
                ...
                <div className={"submission-item "+ (item.result==="ACCEPTED"?'accepted-row':'normal-row')}>
                    <div className="submission-row">
                        ...
                        <div className="info-item" onClick={()=>hanldeToggleShowDetail(index)}>
                            <Button>
                                {showDetailArr[index]?<BsFillCaretDownFill/>:<BsFillCaretUpFill/>} Detail
                            </Button>
                        </div>
                        
                    </div>
                    <div className= {showDetailArr[index]?'show':'hidden'} style={{fontSize:'18px'}}>
                        <SyntaxHighlighter language={languages.filter(x=> Number(item.language)===x.id)[0].value}>
                            {atob(item.code)}
                        </SyntaxHighlighter>
                        <ul>
