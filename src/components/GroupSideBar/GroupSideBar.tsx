...
export const GroupSideBar = () => {
  ...
  const exclusiveLst = useSelector((state: RootState) => {
    const group = state.chat.groupUsers.filter(
  ...
  const handleShowMembers = () => {
    if (currentGroupMode !== GROUP_MODE.POST_LARGE) {
      dispatch(
        setGroupMode({ groupId: currentGroupId, mode: GROUP_MODE.POST_LARGE })
      );
    }
  };
  ...
  return (
    <div className='side-bar-content'>
      <div className='sb-greeting'>
        {myUsername
          ? 'Wellcome ' + getFixedUsername(myUsername)
          : "You're not logined"}
      </div>
      {groups
        ? groups.map(group => (
            <div key={group.id}>
              ...
              >
                <img className='control-item' src={getAvatarLink(group.name)} />
              ...
              {currentGroupId === group.id ? (
                <div>
                  ...
                  >
                    # <FcConferenceCall /> All Member
                  </div>
                  <div className='sb-item' onClick={handleShowFriendsModal}>
                    # <FcPlus /> Add Member
                  </div>
                </div>
              ) : (
      ...
      <FriendListModal
        handleChooseItem={handleAddUserToGroup}
        exlusiveList={exclusiveLst}
      />
    </div>
  );
};
