...
export const ThreadSummaryList = () => {
  const threadList = useSelector(
    (state: RootState) => state.threadList.threadList
  );
        ...
        <tbody>
          {threadList.map((threadData: Thread) => (
            ...
            >
              ...
              <td>
                <div>
                  ...
                  <div style={{ color: 'gray' }}>
                    Posted by: {getFixedUsername(threadData.author.username)} at{' '}
                    {new Date(threadData.createdAt).toUTCString()}
                  </div>
                </div>
              ...
              <td>
                <div className='d-flex-center'>
                  <FcLike size={22} className='like-number' />
                  <span className='like-number'>{threadData.likes}</span>
                </div>
              </td>
            </tr>
          ))}
