  ...
  return (
    <div className='chat-list-item'>
      <div
        key={item.sessionId}
        className={
          currentSessionIdP2P === item.sessionId
            ? 'chat-item chosen'
            : 'chat-item'
        }
        onClick={() =>
          handleLoadChatSession(item.sessionId, item.partner, item.unseenCnt)
        }
      >
        <img className='chat-item-avatar' src={getAvatarLink(item.partner)} />
        <div style={{ paddingLeft: '10px' }}>
          <span style={{ color: 'white' }}>
            {getFixedUsername(item.partner)}
          </span>
          <div>
            {item.lastMessage
              ? getSummary(
                  getFixedUsername(item.lastMessage.authorId) +
                    ': ' +
                    msgContent,
                  30
                )
      ...
      {item.unseenCnt > 0 ? (
        <div className='unseen-msg'>{item.unseenCnt}</div>
      ) : (
        ''
      )}
    </div>
