...
export const ContestStanding = ({contestId, problems}: Props)=>{
    ...
    useEffect(()=>{
        if(!contestId) return;
        const start = (page-1)*itemPerPage, end = page * itemPerPage;
    ...
    if(contestStanding && ctlPagination) return (<div>
        ...
        <thead>
             <tr>
                ...
                {problems.map((item:string)=> <th className="td-item">{item}</th>)}
             </tr>           
         </thead>
         <tbody>
            ...
            <tr>
                ...
                <td className="td-name" ><span className="red">{getFixedUsername(item.username)}</span></td>
                <td className="td-item"><span className="blue">{item.totalScore}</span></td>
                {item.scores.map((x:any)=> 
                <td className="td-item">
                    <div className="green">{x.score>0?x.score:''}</div>
                    <div>
                        <span className="gray">{x.time>0?x.time:''}</span>
                        <span className="red">{x.failedCnt > 0?x.failedCnt:''}</span>
                    </div>
                </td>)}
            </tr>)}
         </tbody>
